...
const registerUserType = z.object({
	username: z.string({ message: "Username not provided" }).trim().max(20, "Maximum length of username should be 20"),
	password: z.string({ message: "Password not provided" }).trim().min(8, "Minimum length of password should be 6").max(20, "Maximum length of password should be 20"),
	userType: z.enum(["creator", "labeller"], { message: "Can be creator or labeller" })
})
				...
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									<FormLabel className="text-gray-300">Type of user</FormLabel>
									<FormControl>
										<select
											className="h-12 md:text-xl bg-gray-700 border-gray-600 text-white placeholder-gray-400"
											{...field}
